...
export default function Create() {
    const { data, setData, post, processing, errors } = useForm({
        ...
        fecha_vencimiento: '', activo: true, archivo: null,
    });
    ...
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold text-gray-800">Nueva publicacion</h2>}>
                    ...
                    <form onSubmit={submit} className="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea rows={5} value={data.contenido} onChange={e => setData('contenido', e.target.value)}
                                className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
                            {errors.contenido && <p className="mt-1 text-xs text-red-600">{errors.contenido}</p>}
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Fecha vencimiento <span className="text-gray-400">(vacío = permanente)</span></label>
                                <input type="date" value={data.fecha_vencimiento} onChange={e => setData('fecha_vencimiento', e.target.value)}
                                    className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
                            </div>
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Archivo adjunto (PDF)</label>
                            <input type="file" accept="application/pdf" onChange={e => setData('archivo', e.target.files[0])}
